...
`TRIGGER_INPUT_DATA` is the ID of the asset to fetch the price for. This input is formatted as a `bytes32` string using the `cast format-bytes32-string` command before being passed to the component.
...
2. The `wasi-exec` command in the [`Makefile`](https://github.com/Lay3rLabs/wavs-foundry-template/tree/v0.3.0/Makefile#L39-L43), which passes input data when testing WAVS components via ``--input `cast format-bytes32-string $(TRIGGER_INPUT_DATA)` ``. This simulates an Ethereum event during local execution.
