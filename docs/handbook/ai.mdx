...
The WAVS Foundry Template contains built-in AI rulefiles for creating components with minimal prompting. These rulefiles are an experimental feature and may not work as expected every time. Components created with AI should not be used in production without thorough review and testing.

## Claude Code

Make sure to follow the [Claude Code installation instructions](https://docs.anthropic.com/en/docs/claude-code/getting-started) to install Claude Code and link your account.

The Claude rulefile is `claude.md` and contains instructions for Claude on how to create a component. Learn more about Claude rulefiles: https://docs.anthropic.com/en/docs/claude-code/memory
...
The Cursor rulefiles are located in the `.cursor/rules` directory. When using Cursor, always attach `component-rules.mdc` file to the chat with your prompt.

```sh Chat
# Attach the rulefile to the chat
...
While the Cursor agent is supposed to always have the provided rulefiles in context, it sometimes ignores the files unless you attach them to the chat. This is a known issue.
...
This prompt will create a component that takes a prompt as input, sends an api request to OpenAI, and returns the response. This example utilizes the OpenAI API with a seed to make the response more deterministic. Please note that while OpenAI models can use seeds, they are not guaranteed to be 100% deterministic. This example is for demonstration purposes and should not be used in production.
...
cp .env.example .env
# place your API key in the .env file (private variables must be prefixed with WAVS_ENV_)
...
5. The agent will start by planning it's component and will create a `plan.md` file. The agent will then make a new component and files according to this plan.


6. The agent will test it's component for errors by running validation tests using `make validate-component COMPONENT=your-component`.
...
- Coding agents can sometimes try to over-engineer its fixes for errors. If you feel it is not being productive, it may be beneficial to start fresh. You may need to adjust your prompt.
...
This repo is designed to be used with short prompts for simple components. However, often times, coding agents will do better with more context.
