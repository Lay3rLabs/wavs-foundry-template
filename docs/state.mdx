...
WAVS provides components with an optional local data directory that can be used for caching. This storage method should only be used to cache immutable data as a performance optimization and not as a replacement for external state stores.
