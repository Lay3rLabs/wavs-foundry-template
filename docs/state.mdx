...
When state persistence is necessary, it should utilize an external immutable source (like Ethereum or IPFS) which are optimized for this functionality and can be deterministically referenced. This design bypasses the consensus errors that can arise from mutable state between operators.
